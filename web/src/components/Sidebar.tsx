      ...
      <div className="flex-1 overflow-y-auto p-2 pt-0 space-y-0.5">
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="flex items-center gap-1">
                ...
                {!!a.pinned && <Pin size={10} className="shrink-0 text-zinc-500" />}
                ...
                {!!a.hidden && <EyeOff size={10} className="shrink-0 text-zinc-600" />}
              </span>
              {isWorking ? (
        ...
        {hiddenCount > 0 && (
          <button
            ...
            className="flex w-full items-center gap-2 rounded-lg px-2.5 py-1.5 text-[11px] text-zinc-600 hover:text-zinc-400"
          ><EyeOff size={11} /> {showHidden ? "숨긴 봇 감추기" : `숨긴 봇 ${hiddenCount}개 보기`}</button>
        )}

        {/* 그룹채팅 — 여러 봇이 하나의 대화에 참여 */}
        <div className="px-1 pt-3 pb-1 text-[10px] font-semibold uppercase tracking-wider text-zinc-600">그룹</div>
        {groups.map((g) => (
          <div
            key={g.id}
            className={`flex cursor-pointer items-center gap-2 rounded-lg px-2.5 py-2 text-sm ${
              g.id === activeGroupId ? "bg-zinc-800 text-zinc-100" : "text-zinc-300 hover:bg-zinc-900"
            }`}
          ...
          >
            <span className="flex shrink-0 -space-x-1.5">
              {g.members.slice(0, 3).map((m) => (
                <AgentIcon key={m.id} name={m.name} seed={m.avatar} size={17} className="rounded-full ring-1 ring-zinc-950" working={working?.[m.id] !== undefined} />
              ))}
            </span>
            <span className="min-w-0 flex-1">
              <span className="block truncate">{g.name}</span>
              <span className="block truncate text-[10px] leading-tight text-zinc-500">{g.members.map((m) => m.name).join(" · ")}</span>
            </span>
          </div>
        ))}
        {groupForm ? (
          <div className="rounded-lg border border-zinc-800 p-2.5 space-y-2">
            ...
            <div className="max-h-40 space-y-1 overflow-y-auto">
              {agents.filter((a) => !a.hidden).map((a) => (
                <label key={a.id} className="flex cursor-pointer items-center gap-2 rounded px-1 py-0.5 text-xs text-zinc-300 hover:bg-zinc-900">
                  <input
                    type="checkbox"
            ...
            <div className="flex gap-1.5">
              <button onClick={() => setGroupForm(false)} className="flex-1 rounded-lg bg-zinc-800 py-1.5 text-[11px] text-zinc-400">취소</button>
              <button
                onClick={createGroup}
                disabled={!groupName.trim() || picked.size < 2}
                className="flex-1 rounded-lg bg-zinc-100 py-1.5 text-[11px] font-medium text-zinc-900 disabled:opacity-40"
