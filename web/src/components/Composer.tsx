...
export function Composer({
  ...
  const chip = (m: Mode, label: string, active_cls: string) => (
    <button
      ...
      className={`shrink-0 whitespace-nowrap rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${
        mode === m ? active_cls : "bg-stone-200 text-stone-600 hover:bg-stone-300 hover:text-stone-800"
      }`}
    >
      {label}
    </button>
  ...
  return (
    <div className="rounded-3xl border border-stone-200 bg-white/80 p-3 shadow-[0_4px_20px_rgba(0,0,0,0.05)] backdrop-blur">
      {attachments.length > 0 && (
        <div className="mb-2 flex flex-wrap gap-2 px-1">
          {attachments.map((a, i) => (
            <div key={i} className="relative">
              {a.mime.startsWith("image/") ? (
                <img src={a.url} className="h-14 w-14 rounded-lg object-cover border border-stone-300" />
              ) : (
                <span className="flex h-14 items-center rounded-lg border border-stone-300 bg-stone-200 px-2 text-[10px]">{a.name}</span>
              )}
              <button
                className="absolute -right-1 -top-1 rounded-full bg-stone-300 px-1 text-[10px] text-stone-700 hover:bg-red-600"
      ...
      <textarea
        ref={ref}
        className="w-full resize-none bg-transparent px-2 py-1 text-base md:text-[15px] outline-none placeholder:text-stone-400 max-h-48"
        rows={1}
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2">
        <button
          ...
          className="shrink-0 rounded-full p-1.5 text-stone-500 hover:bg-stone-200 hover:text-stone-800"
          title="이미지/파일 첨부"
        >
        ...
        <select
          className="shrink-0 rounded-full bg-stone-200 px-2 py-1.5 text-xs text-stone-700 outline-none hover:bg-stone-300 max-w-[110px]"
          value={personaId}
        ...
        <button
          onClick={toggleVoice}
          className={`rounded-full p-1.5 transition-colors ${listening ? "bg-red-600 text-white animate-pulse" : "text-stone-500 hover:bg-stone-900 hover:text-stone-800"}`}
