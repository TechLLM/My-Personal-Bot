...
function ProviderRow({ p, onChanged }: { p: ProviderCard; onChanged: () => void }) {
  ...
  const status = !p.enabled ? { t: "비활성", c: "text-zinc-500", dot: "bg-zinc-600" }
    ...
    : { t: "인증 필요", c: "text-zinc-500", dot: "bg-zinc-600" };

  return (
    <div className={`rounded-xl border px-3 py-2.5 ${p.enabled ? "border-zinc-800 bg-zinc-900/60" : "border-zinc-800/50 bg-zinc-900/20 opacity-60"}`}>
      <div className="flex items-center gap-2.5">
        <button
          onClick={() => api.toggleProvider(p.id).then(onChanged)}
          className={`relative h-4.5 w-8 shrink-0 rounded-full transition-colors ${p.enabled ? "bg-emerald-500/80" : "bg-zinc-700"}`}
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <span className="rounded bg-zinc-800 px-1.5 py-0.5 text-[9px] text-zinc-400">{p.authLabel}</span>
          </div>
          <div className="mt-0.5 flex items-center gap-1.5">
            <span className={`h-1.5 w-1.5 rounded-full ${status.dot}`} />
            <span className={`text-[10px] ${status.c}`}>{status.t}</span>
            {p.staticModels.length > 0 && <span className="text-[10px] text-zinc-600">· 모델 {p.staticModels.length}개</span>}
          </div>
        </div>
        <div className="flex shrink-0 items-center gap-1">
          {p.authType === "apikey" && (
            <button onClick={() => setKeyOpen(!keyOpen)} className="rounded-lg p-1.5 text-zinc-500 hover:bg-zinc-800 hover:text-zinc-200" title={p.hasManualKey ? "키 교체" : "API 키 입력"}>
              <KeyRound size={13} />
            </button>
          )}
          <button onClick={test} disabled={testing || !p.enabled} className="rounded-lg p-1.5 text-zinc-500 hover:bg-zinc-800 hover:text-zinc-200 disabled:opacity-40" title="연결 테스트">
            {testing ? <Loader2 size={13} className="animate-spin" /> : <Zap size={13} />}
          </button>
          {p.custom && (
            <button onClick={() => api.deleteCustomProvider(p.id).then(onChanged)} className="rounded-lg p-1.5 text-zinc-600 hover:bg-zinc-800 hover:text-red-400" title="삭제">
              <Trash2 size={13} />
            </button>
          )}
        </div>
      </div>
      {p.doc && p.enabled && !p.authed && <p className="mt-1.5 text-[10px] text-zinc-600">{p.doc}</p>}
      {keyOpen && (
        <div className="mt-2 flex gap-1.5">
          <input type="password" className="flex-1 rounded-lg bg-zinc-800 px-2.5 py-1.5 text-xs outline-none" placeholder="API 키 입력" value={key} onChange={(e) => setKey(e.target.value)} onKeyDown={(e) => e.key === "Enter" && saveKey()} autoFocus />
          <button onClick={saveKey} className="rounded-lg bg-zinc-100 px-2.5 text-xs font-medium text-zinc-900">저장</button>
        </div>
      )}
...
export function SettingsModal({ models, onClose }: { models: Model[]; onClose: () => void }) {
  ...
  const Field = ({ k, label, ph }: { k: string; label: string; ph?: string }) => (
    <label className="block">
      ...
      <input className="mt-1 w-full rounded-lg bg-zinc-800 px-2.5 py-1.5 text-xs outline-none" value={s[k] ?? ""} placeholder={ph} onChange={(e) => update({ [k]: e.target.value })} />
    </label>
  );
  const Input = "w-full rounded-lg bg-zinc-800 px-2.5 py-1.5 text-xs outline-none";
  const Btn = "rounded-lg bg-zinc-100 px-2.5 py-1 text-xs font-medium text-zinc-900 hover:bg-white";
  const Sub = "rounded-lg bg-zinc-800 px-2.5 py-1 text-xs text-zinc-300 hover:bg-zinc-700";
  const H = ({ children }: { children: React.ReactNode }) => <h3 className="mb-3 text-sm font-semibold">{children}</h3>;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div className="flex h-[80vh] w-full max-w-3xl overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-950" onClick={(e) => e.stopPropagation()}>
        {/* ─── 좌측 섹션 사이드바 ─── */}
        <nav className="flex w-44 shrink-0 flex-col border-r border-zinc-800/80 bg-zinc-950 p-2">
          ...
          {SECTIONS.map(({ id, label, icon: Icon }) => (
            <button
              ...
              className={`mb-0.5 flex items-center gap-2 rounded-lg px-2.5 py-2 text-left text-xs transition-colors ${section === id ? "bg-zinc-800 text-zinc-100" : "text-zinc-500 hover:bg-zinc-900 hover:text-zinc-300"}`}
            >
          ...
          <div className="mt-auto px-2 pb-1 text-[10px] text-zinc-700">MyBot 로컬 설정</div>
        </nav>
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <div className="flex-1 overflow-y-auto p-5">
            ...
            {section === "providers" && (
              <div>
                ...
                {/* 커스텀 프로바이더 등록 */}
                <div className="mt-4 rounded-xl border border-dashed border-zinc-800 p-3">
                  <div className="mb-2 flex items-center gap-1.5 text-xs font-medium text-zinc-400"><Plus size={12} /> 커스텀 프로바이더 (OpenAI 호환)</div>
                  ...
                  <p className="mt-1.5 text-[10px] text-zinc-600">Ollama: http://127.0.0.1:11434/v1 · LM Studio: http://127.0.0.1:1234/v1</p>
                </div>

                <label className="mt-4 block">
                  <span className="text-xs text-zinc-400">기본 AI 모델 — 새 봇에 자동 적용</span>
                  <select className="mt-1 w-full rounded-lg bg-zinc-800 px-2.5 py-1.5 text-xs outline-none" value={s.default_model ?? ""} onChange={(e) => update({ default_model: e.target.value })}>
                    <option value="">자동 (인증된 첫 프로바이더)</option>
                    {models.map((m) => <option key={m.id} value={m.id}>{m.providerName} · {m.label}</option>)}
                  </select>
                  <span className="mt-0.5 block text-[10px] text-zinc-600">인증된 프로바이더의 모델만 표시됩니다 — 목록에 없으면 위에서 프로바이더를 연결하세요</span>
                </label>
              </div>
            )}
            ...
            {section === "agents" && (
              <div>
                ...
                <div className="space-y-1.5">
                  {agents.map((a) => (
                    <div key={a.id} className="rounded-lg bg-zinc-900 px-2.5 py-2 text-xs">
                      <div className="flex items-center gap-2">
                        ...
                        <select className="max-w-[160px] truncate rounded bg-zinc-800 px-1 py-0.5 text-[10px] text-zinc-400 outline-none" value={a.model ?? ""} title={a.model_label ?? a.model ?? ""} onChange={(e) => api.updateAgent(a.id, { model: e.target.value }).then(load)}>
                          {models.map((m) => <option key={m.id} value={m.id}>{m.providerName} · {m.label}</option>)}
                  ...
                  {!agents.length && <p className="text-xs text-zinc-600">아직 봇이 없습니다 — 대장에게 지시하면 자동 생성됩니다</p>}
                </div>
                <div className="mt-3 space-y-1.5">
                  <div className="flex gap-1.5">
                    <input className={`${Input} flex-1`} placeholder="봇 이름 (예: 리서치봇)" value={aName} onChange={(e) => setAName(e.target.value)} />
                    <select className="w-40 rounded-lg bg-zinc-800 px-2 py-1.5 text-xs outline-none" value={aModel} onChange={(e) => setAModel(e.target.value)} title="비우면 기본 AI 모델 적용">
            ...
            {section === "routines" && (
              <div>
                ...
                <div className="space-y-1.5">
                  {routines.map((r) => (
                    <div key={r.id} className="rounded-lg bg-zinc-900 px-2.5 py-2 text-xs">
                      <div className="flex items-center gap-2">
                ...
                <div className="mt-3 space-y-1.5">
                  <div className="flex gap-1.5">
                    ...
                    <select className="rounded-lg bg-zinc-800 px-2 py-1.5 text-xs outline-none" value={rAgent} onChange={(e) => setRAgent(e.target.value)} title="담당 봇">
            ...
            {section === "search" && (
              <div>
                ...
                <select className="mb-3 w-full rounded-lg bg-zinc-800 px-2.5 py-1.5 text-xs outline-none" value={s.search_provider ?? "auto"} onChange={(e) => update({ search_provider: e.target.value })}>
            ...
            {section === "tools" && (
              <div className="space-y-6">
                <div>
                  ...
                  <div className="space-y-1.5">
                    {workspaces.map((w) => (
                      <div key={w.id} className="flex items-center gap-2 rounded-lg bg-zinc-900 px-2.5 py-1.5 text-xs">
                ...
                <div>
                  ...
                  <div className="space-y-1.5">
                    {skills.map((sk) => (
                      <div key={sk.id} className="flex items-center gap-2 rounded-lg bg-zinc-900 px-2.5 py-1.5 text-xs">
                ...
                <div>
                  ...
                  <div className="space-y-1.5">
                    {personas.map((p) => (
                      <div key={p.id} className="flex items-center gap-2 rounded-lg bg-zinc-900 px-2.5 py-1.5 text-xs">
                        <AgentIcon name={p.name} size={13} className="text-zinc-500" />
                        <span>{p.name}</span>
                        <span className="flex-1 truncate text-zinc-500">{p.prompt || "(기본)"}</span>
                        {!p.builtin && <button className="text-zinc-600 hover:text-red-400" onClick={() => mybotFetch(`/api/personas/${p.id}`, { method: "DELETE" }).then(load)}>삭제</button>}
                      </div>
                    ))}
                  </div>
                  <div className="mt-2 space-y-1.5">
            ...
            {section === "browser" && (
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-1.5">
                    {sites.map((st) => (
                      <div key={st.id} className="flex items-center gap-2 rounded-lg bg-zinc-900 px-2.5 py-1.5 text-xs">
            ...
            {section === "notify" && (
              <div>
                <H>결과 알림</H>
                <p className="mb-3 text-[11px] text-zinc-600">기본은 채팅창에만 표시됩니다. 체크한 채널로 답변·루틴 결과를 함께 받습니다.</p>
                <div className="space-y-2.5">
            ...
            {section === "memory" && (
              <div>
                ...
                <div className="space-y-1">
                  {shownMemories.map((m) => (
                    <div key={m.id} className="flex items-center gap-2 rounded-lg bg-zinc-900 px-2.5 py-1.5 text-xs text-zinc-400">
                      <span className="flex-1 truncate">{m.content}</span>
                      <button className="shrink-0 text-zinc-600 hover:text-red-400" onClick={() => {
                ...
                {memories.length > 5 && (
                  <button onClick={() => setMemOpen(!memOpen)} className="mt-2 flex items-center gap-1 text-[11px] text-zinc-500 hover:text-zinc-300">
          ...
          <div className="flex items-center gap-3 border-t border-zinc-800 px-5 py-3">
            <button onClick={saveAll} className="rounded-lg bg-zinc-100 px-4 py-1.5 text-xs font-semibold text-zinc-900 hover:bg-white">저장</button>
