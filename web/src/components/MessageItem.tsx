  ...
  if (m.role === "user") {
    return (
      <div className="group flex flex-col items-end gap-1">
        {editing ? (
          <div className="w-full rounded-2xl bg-stone-200 p-3">
            <textarea
            ...
            <div className="flex justify-end gap-2 mt-1">
              <button className="text-xs text-stone-600 hover:text-stone-800" onClick={() => setEditing(false)}>취소</button>
              <button
                className="text-xs bg-stone-900 text-white rounded-full px-3 py-1 font-medium"
                onClick={() => { onEdit?.(m, editText); setEditing(false); }}
              >저장 후 재전송</button>
            </div>
          </div>
        ) : (
          <div className="max-w-[80%] rounded-2xl bg-stone-200 px-4 py-2.5 text-[15px] whitespace-pre-wrap break-words">
            {m.attachments && (JSON.parse(m.attachments) as { url: string; mime: string; name: string }[]).map((a, i) =>
              a.mime.startsWith("image/") ? (
                ...
                <div key={i} className="mb-2 flex items-center gap-1 rounded-lg bg-stone-300 px-2 py-1 text-xs"><FileText size={11} /> {a.name}</div>
              ),
            )}
            {m.content}
          </div>
        )}
        <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity text-stone-500">
          {sibs && (
            <span className="flex items-center gap-1 text-xs">
              <button onClick={() => onSelectSibling?.(m, -1)} className="hover:text-stone-800">‹</button>
              {(m.sibling_index ?? 0) + 1}/{m.sibling_count}
              <button onClick={() => onSelectSibling?.(m, 1)} className="hover:text-stone-800">›</button>
            </span>
          )}
          <button className="text-xs hover:text-stone-800" onClick={() => { setEditText(m.content); setEditing(true); }}>편집</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="group flex flex-col gap-2">
      {(m.reasoning || streaming) && m.reasoning !== null && m.reasoning !== "" && (
        <div className="rounded-xl border border-stone-200 bg-white/50">
          <button
            className="flex w-full items-center gap-2 px-3 py-2 text-xs text-stone-600 hover:text-stone-800"
          ...
          {showReasoning && (
            <div className="border-t border-stone-200 px-3 py-2 text-xs italic text-stone-500 whitespace-pre-wrap max-h-64 overflow-y-auto">
              {m.reasoning}
            </div>
          )}
        </div>
      ...
      {meta?.type === "team" && meta.status === "pending" && meta.agents && (
        <div className="mt-1 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5">
            ...
            {meta.agents.map((a, i) => (
              <label key={i} className="flex items-start gap-2 rounded-lg bg-white/60 px-2.5 py-2 text-xs cursor-pointer">
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block truncate text-stone-500">{a.role}</span>
                  <span className="block text-stone-600">작업: {a.task}</span>
                </span>
              </label>
            ))}
          </div>
          <div className="mt-2 flex gap-2">
            <button
              className="rounded-full bg-amber-600 px-3 py-1 text-xs font-medium text-white disabled:opacity-40 hover:bg-amber-600"
              disabled={!sel.size}
              onClick={() => onTeamConfirm?.(m, meta.agents!.filter((_, i) => sel.has(i)))}
            >선택한 봇 {sel.size}개로 실행</button>
            <button
              className="rounded-full bg-stone-200 px-3 py-1 text-xs text-stone-600 hover:text-stone-800"
          ...
          <div className="space-y-1 text-xs text-stone-500">
            {meta.agents.map((a, i) => (
              <div key={i} className="flex items-center gap-1.5">
                ...
                <span className="text-stone-400"> — {(a.task ?? "").slice(0, 60)}{(a.task ?? "").length > 60 ? "…" : ""}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {meta?.events && meta.events.length > 0 && (
        <div className="mt-1 rounded-xl border border-stone-200 bg-white/40 px-3 py-2">
      ...
      {meta && meta.type !== "team" && (meta.sources?.length ?? 0) > 0 && (
        <div className="mt-1 rounded-xl border border-stone-200 bg-white/40 px-3 py-2">
          <div className="text-xs font-medium text-stone-600 mb-1">출처 {meta.sources!.length}개 · 검색 {meta.steps}회</div>
      ...
      <div className="flex items-center gap-3 text-stone-400 text-xs opacity-0 group-hover:opacity-100 transition-opacity">
        {sibs && (
        ...
        <button className="hover:text-stone-800" onClick={() => onRegenerate?.(m)}>재생성</button>
        <button
          className="hover:text-stone-800"
          title="읽어주기"
          onClick={() => {
