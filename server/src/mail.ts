...
export async function mailList(args: Record<string, unknown>): Promise<string> {
  ...
  return (await withMailbox(mailbox, async (client) => {
    ...
    return `${mailbox} — 조건 일치 ${uids.length}건 중 최근 ${rows.length}건 (본문은 mail_read(uid)로 읽으세요)\n${rows.join("\n")}`;
  })) as string;
}

export async function mailRead(args: Record<string, unknown>): Promise<string> {
  const uid = String(args.uid ?? args.id ?? "").trim();
  if (!uid) return "오류: 읽을 메일의 uid가 없습니다 — mail_list가 알려준 uid를 넣으세요";
  const mailbox = String(args.mailbox || "INBOX");
  return (await withMailbox(mailbox, async (client) => {
    const msg = await client.fetchOne(uid, { envelope: true, bodyStructure: true }, { uid: true });
    if (!msg) return `uid ${uid} 메일을 찾지 못했습니다 — mail_list로 uid를 다시 확인하세요`;
    ...
    const trimmed = body.slice(0, 8000);
    return `${head}\n\n${trimmed || "(본문 없음)"}${body.length > 8000 ? `\n…(잘림 — 전체 ${body.length}자)` : ""}`;
  })) as string;
}
