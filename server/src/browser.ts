...
export const BROWSER_TOOLS = [
  ...
  { type: "function", function: { name: "bsk", description: "BrowserSkill — 사용자의 실제 로그인된 Chrome/Edge를 제어합니다 (bsk CLI). 내장 browser_*·ego_run으로 안 되는 사이트에 쓰세요 — 실제 사용자 브라우저라 로그인 세션·봇 감지 문제가 없습니다. cmd에 bsk 서브커맨드를 통째로 넣으세요. 세션은 사용자 화면을 가로채지 않도록 자동으로 백그라운드(--no-focus)로 시작됩니다. 기본 흐름: 1) 'session start --json' → session_id 획득 2) 'navigate <url> --session <id>' 3) 'observe --session <id>' → @eN 참조 확인 4) 'click @e3 --session <id>' / 'fill @e3 --value \"텍스트\" --session <id>' 5) 작업 끝에 반드시 'session stop <id>'. 사람 개입 필요하면 'request-help --session <id> --prompt \"설명\"'. 허용 명령: session/navigate/observe/click/fill/press/select/hover/scroll-to/wheel/focus/blur/snapshot/get-html/screenshot/tab/request-help/status/browsers.", parameters: { type: "object", properties: { cmd: { type: "string", description: "bsk 서브커맨드 전체 — 예: 'observe --session abc123'" } }, required: ["cmd"] } } },
];
