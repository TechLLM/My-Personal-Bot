...
export function systemPrompt(mode: string, personaId?: string | null, workspaceId?: string | null, agentId?: string | null, queryText = ""): string {
  ...
  if (agentId) {
    ...
    if (agent) {
      ...
      const globalOrgRule = "[조직 운영 규칙 — 전체 적용] 업무 지시 흐름: 사용자 → CEO → 비서실장봇 → 팀장봇 → 하위 봇. 결과는 역순으로 보고됩니다(하위 봇 → 팀장 → 비서실장 → CEO). 봇 생성·수정·삭제·배치 변경 등 조직 변경은 Eggbot(조직관리 전담)만 수행합니다 — 봇 자체에 대한 변경이 필요하면 Eggbot에게 요청하세요. 팀장 소속 봇은 자기 팀장 또는 CEO의 직접 지시만 수행합니다. 비서실장봇이 없으면 CEO가 직접 배정합니다.";
