...
const BOSS_ROLE = "당신은 MyBot의 CEO(총괄 관리자) 봇입니다. 사용자의 모든 업무 지시를 받는 총괄 책임자이며, 모든 봇에 대한 전체 권한을 가집니다. 스스로 도구(웹검색·파일·브라우저·MCP)를 사용해 직접 수행하거나, 필요하면 전문 역할 봇들에게 분배하고 결과를 종합해 보고합니다. 봇 관리: agent_create로 새 전문 봇 생성, agent_list로 전체 봇 현황 확인, agent_direct로 임의 봇에게 즉시 업무 지시(결과를 받아 종합), agent_update로 봇의 역할·모델 수정 및 팀장 지정/해제(lead 옵션)·하위 봇 한도 조정(max_children, 기본 4), agent_delete로 불필요한 봇 정리. 조직이 커지면 분야별 팀장을 지정하세요 — 팀장은 자기 하위 봇을 생성·지시·취합해 당신에게 보고합니다. 사용자가 반복적·정기적 작업을 요청하면 routine_add 도구로 예약 작업으로 등록하세요 — 일회성 실행으로 처리하지 마세요. 이전 대화와 기억한 맥락을 바탕으로 업무의 연속성을 유지하세요.";
...
export function ensureBossAgent(): Agent {
  ...
  if (!a) {
    ...
    const role = a.role_prompt.includes("MyBot의 CEO")
      ? BOSS_ROLE
      : `${a.role_prompt}\n\n[CEO 권한] 당신은 모든 봇의 관리자입니다. agent_create(봇 생성), agent_list(봇 현황), agent_direct(봇에게 즉시 지시), agent_update(역할·모델 수정·lead 옵션으로 팀장 지정/해제·max_children으로 하위 봇 한도 조정), agent_delete(봇 삭제), routine_add(예약 등록) 도구를 사용할 수 있습니다. 팀장은 자기 하위 봇을 생성·지시·취합해 당신에게 보고합니다.`;
...
export const MANAGE_TOOLS = [
  ...
  { type: "function", function: { name: "agent_update", description: "봇의 역할 지침·모델을 수정하거나 팀장으로 지정/해제합니다 (lead·max_children 지정은 관리자만 가능)", parameters: { type: "object", properties: { name: { type: "string" }, role: { type: "string" }, model: { type: "string" }, lead: { type: "boolean", description: "true=팀장 지정, false=팀장 해제 (관리자만)" }, max_children: { type: "number", description: "팀장이 생성 가능한 하위 봇 한도 (관리자만, 기본 4)" } }, required: ["name"] } } },
...
export async function callBuiltin(name: string, args: Record<string, unknown>, agentId?: string | null, signal?: AbortSignal, depth = 0, emit?: (ev: any) => void): Promise<string> {
  ...
  if (name === "agent_update") {
    ...
    db.prepare("UPDATE agents SET role_prompt = ?, model = ?, is_lead = ?, max_children = ? WHERE id = ?")
    ...
    return `봇 수정됨: ${target.name}${args.lead !== undefined && (!caller || caller.is_boss) ? (args.lead ? " — 팀장 지정" : " — 팀장 해제") : ""}${args.max_children !== undefined && (!caller || caller.is_boss) ? ` — 하위 봇 한도 ${mc}개` : ""}`;
  }
  ...
  const messages: any[] = [
    {
      ...
      content: `당신은 전문 에이전트 "${agent.name}"입니다.\n역할: ${agent.role_prompt}\n\n[현재 시각] ${new Date().toLocaleString("ko-KR", { timeZone: "Asia/Seoul", year: "numeric", month: "long", day: "numeric", weekday: "long", hour: "2-digit", minute: "2-digit" })} (한국 표준시) — "오늘/최근" 표현과 검색 결과의 연도는 반드시 이 시각 기준으로 판별하세요.\n\n지시받은 작업을 수행하세요. 필요하면 도구(web_search, 브라우저, 파일, MCP)를 사용하세요. 브라우저 도구는 사용자의 로그인 세션을 공유하므로 로그인이 필요한 사이트도 열 수 있습니다.\n\n${isBoss
        ...
        : isLead
          ? "당신은 팀장입니다 — 자기 하위 봇에 대한 관리 권한을 가집니다: agent_create(하위 봇 생성 — 생성된 봇은 당신의 팀 소속, 최대 4개까지. 초과가 필요하면 관리자에게 요청), agent_update·agent_delete(하위 봇만), agent_direct(하위 봇에게 지시하고 결과를 취합해 지시한 쪽에 보고). 한도에 도달하면 더 만들지 말고 있는 봇들에게 지시하세요."
